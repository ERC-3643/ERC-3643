...
describe('UtilityChecker.getTransferStatus', () => {
  describe('When sender is frozen', () => {
    it('should return false', async () => {
      ...
      const result = await utilityChecker.getTransferStatus(token.target, aliceWallet.address, bobWallet.address, 100);
  ...
  describe('When the identity is registered with topics', () => {
    it('should return false', async () => {
      ...
      const result = await utilityChecker.getTransferStatus(token.target, aliceWallet.address, charlieWallet.address, 100);
      expect(result[0]).to.be.equal(true);
      expect(result[1]).to.be.equal(false);
      expect(result[2]).to.be.equal(true);
    });
